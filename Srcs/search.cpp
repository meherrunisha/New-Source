...
void Thread::search() {
  ...
  while (++rootDepth < DEPTH_MAX && !Signals.stop && (!Limits.depth || rootDepth <= Limits.depth))
  {
	  ...
	  widePV = (threads > 3 && rootDepth < 5 * ONE_PLY && mainThread) ? rootMoves.size() : multiPV;

      // MultiPV loop. We perform a full root search for each PV line
      for (PVIdx = 0; PVIdx < widePV && !Signals.stop; ++PVIdx)
      {
          ...
          else if (PVIdx + 1 == widePV || Time.elapsed() > 3000)
              sync_cout << UCI::pv(rootPos, rootDepth, alpha, beta) << sync_endl;
      }
      ...
      if (Limits.use_time_management())
      {
          if (!Signals.stop && !Signals.stopOnPonderhit)
          {
              ...
              int improvingFactor = std::max(200, std::min(700, 480 - 5 * (bestValue - mainThread->previousScore)));
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
                            ...
                            : (tte->bound() & BOUND_UPPER)))
    {
        ...
        // If ttMove is quiet, update killer, history, counter move on TT hit
    ...
    if (   !PvNode
        &&  depth >= 5 * ONE_PLY
		&&  eval + PieceValue[MG][pos.captured_piece_type()] >= beta + 60
...
moves_loop: // When in check search starts from here
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
          ...
          &&  bestValue > VALUE_MATED_IN_MAX_PLY)
      {
          ...
          if (   depth < 16 * ONE_PLY
              ...
              && cmh[pos.moved_piece(move)][to_sq(move)] < 13991 - 222 * depth / ONE_PLY)
              continue;
          ...
          && !captureOrPromotion)
      {
          ...
          int rHist = (hValue + cmhValue + fmValue + fm2Value) / 27000;
    ...
    */
    ...
    // Quiet best move: update killer, history and countermoves
  ...
  // update_stats() updates killer, history, countermove and countermove
